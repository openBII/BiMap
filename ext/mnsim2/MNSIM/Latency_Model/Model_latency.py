...
from ext.mnsim2.MNSIM.Latency_Model.Tile_latency import tile_latency_analysis
from ext.mnsim2.MNSIM.Latency_Model.Pooling_latency import pooling_latency_analysis
from ext.mnsim2.MNSIM.NoC.interconnect_estimation import interconnect_estimation
...
class Model_latency():
    ...
    def calculate_model_latency(self, mode=0):
        ...
        for layer_id in range(len(self.NetStruct)):
            layer_dict = self.NetStruct[layer_id][0][0]
            if layer_id == 0 and layer_dict['type'] != 'fc':
                # for the first layer, first layer must be conv layer
                self.layer_latency_initial()
                output_size = list(map(int, layer_dict['Outputsize']))
                input_size = list(map(int, layer_dict['Inputsize']))
