...
def PIM_Compute_O_Split(input_size, output_size, macro_num, batch_num=1):
    fops = input_size * output_size
    ...
    if input_width * 16 > max_width:
        load_round = math.ceil(input_width * 16 / max_width)
        print("Load Input Round: ", load_round)
    else:
        load_round = 1
        print("Load Input Round: ", load_round)
    ...
    overall_latency = reload_size * reduce_size * (ld_i_latency * load_round * batch_num + ld_w_latency + exe_latency)
    
    print("Overall Latency: ", overall_latency, "ns")
    print("Latency / Batch: ", overall_latency / batch_num, "ns")
    

if __name__ == "__main__":
    PIM_Compute_O_Split(4096, 8, 8, 16)
    PIM_Compute_O_Split(4096, 22, 8, 16)
    PIM_Compute_O_Split(11008, 8, 8, 16)