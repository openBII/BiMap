        ...
        if (request_found) {
            if (m_dram->m_command_meta(req_it->command).is_opening && m_dram->m_command_scopes(req_it->command) == m_row_level) {
                ...
                if (m_is_debug) {
                    ...
                    std::cout << "  └  "
                              << "rank: " << req_it->addr_vec[m_rank_level] << std::endl;
                    std::cout << "  └  "
                              << "bank_group: " << req_it->addr_vec[m_rank_level + 1] << std::endl;
                    std::cout << "  └  "
                              << "bank: " << req_it->addr_vec[m_bank_level] << std::endl;
                    std::cout << "  └  "
                              << "index: " << flat_bank_id << std::endl;
                }
                ...
                if (m_activation_count_table[flat_bank_id].find(row_id) == m_activation_count_table[flat_bank_id].end()) {
                    ...
                    for (auto it = m_activation_count_table[flat_bank_id].begin(); it != m_activation_count_table[flat_bank_id].end(); it++) {
                        if (m_is_debug)
                            std::cout << "  └  "
                                      << "checking row " << it->first << " with count " << it->second << std::endl;
                    ...
                    if (found) {
                        ...
                        if (m_is_debug) {
                            ...
                            std::cout << "  └  "
                                      << "spillover counter: " << m_spillover_counter[flat_bank_id] << std::endl;
                        }
                    ...
                    if (m_is_debug) {
                        ...
                        std::cout << "  └  "
                                  << "threshold: " << m_activation_threshold << std::endl;
                        std::cout << "  └  "
                                  << "count: " << m_activation_count_table[flat_bank_id][row_id] << std::endl;
                    }
