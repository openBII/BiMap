...
class TaskGraph():
    ...
    def add_edge(self, source_id: int, destination_id: int,
                 source_cluster: int = 0, destination_cluster: int = 0,
                 source_position: Shape = None, destination_position: Shape = None,
                 src_size: Shape = None, dst_size: Shape = None,
        ...
        Args:
            ...
            source_cluster: 源结点的边簇序号
            destination_cluster: 目的结点边簇序号
            source_position: 源结点的边簇中准备通过此连接发送的数据形状起始位置
            destination_position: 目的结点的边簇中准备接收此连接发送的数据形状起始位置
            src_size: 源结点的边簇中准备通过此连接发送的数据形状大小
            dst_size: 目的结点的边簇中准备通过此连接接收的数据形状大小
        ...
        edge = Edge(in_task, out_task, rearrange_info)

        # TODO(weihao): 把cluster的冗余逻辑改一下
        if source_position is None:
            source_position = Shape()
        if destination_position is None:
            destination_position = Shape()
        if src_size is None:
            src_size = in_task.output_clusters[source_cluster].shape
        if dst_size is None:
            dst_size = out_task.input_clusters[destination_cluster].shape

        in_task.add_output_edge(edge, source_cluster,
                                source_position, src_size)
        out_task.add_input_edge(edge, destination_cluster,
                                destination_position, dst_size)
        return edge
    ...
    def topologize(self) -> None:
        ...
        for node_id in self._nodes.keys():
            ...
            for ic in self._nodes[node_id].input_clusters:
                in_degrees[node_id] += len(ic.all_enable_edges)
        ...
        while node_zero_in:
            ...
            for oc in nodes_topo[u].output_clusters:
                for occ in oc.all_enable_edges:
                    out_task_id = occ.out_task.id
