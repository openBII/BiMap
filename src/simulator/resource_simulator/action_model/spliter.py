...
from task_rabbit.task_model.ctask_block import CADDTaskBlock
from task_rabbit.task_model.stask_block import SITaskBlock
...
from task_rabbit.task_model.stask_block import SBTaskBlock


class Spliter(object):
    def __init__(self, base_node_id: int, shape: Shape,
                 split_mode_list: list, graph: TaskGraph, aligned_num=None):
        self.graph = graph
        ...
        self.si_node_behind_cadd = []  # new SI nodes right after CADD node
    ...
    def add_addition_blocks(self):
        ...
        # link new_c_block to original base_node's out task
            ...
            for edge_cluster in bias_node.output_clusters:
                for edge in edge_cluster.all_edges:
                    if edge.post_task is self.base_node:
                        ...
                        new_c_block.input_clusters[0].add_edge(
        ...
        for i in range(self.shape.nr):
            ...
            self.si_node_behind_cadd.append(new_s_block)
            # link new_s_block to base_node
            new_edge = Edge(self.base_node, new_s_block)
            self.base_node.output_clusters[0].add_edge(Shape(), deepcopy(
                self.base_node.output_clusters[0].shape), new_edge)
            new_s_block.input_clusters[0].add_edge(Shape(), deepcopy(
            ...
            new_c_block.add_input_edge(i + 1, Shape(), deepcopy(
                new_c_block.input_clusters[1].shape), new_edge)
            new_s_block.output_clusters[0].add_edge(Shape(), deepcopy(
                new_s_block.output_clusters[0].shape), new_edge)

    def copy_node_without_edge(self):
        """
    ...
    def remove_reduction_links_between_new_node_and_si(self):
        ...
        """
        for node in self.si_node_behind_cadd:
    ...
    def check(self):
        if self.shape.nky != 1 or self.shape.nkx != 1:
            raise ValueError('Cannot split kx or ky!')
        for idx in range(4):
    ...
    def ave_split_func_f(self, total_length, split_num, aligned_num=None):
        ...
        if aligned_num:
            ...
            if split_num > total_length:
                raise ValueError('split_num is too large')
