...
import random
import copy
from pyecharts.options import LabelOpts, ItemStyleOpts, LineStyleOpts, InitOpts
from pyecharts.charts import Graph
from pyecharts import options as opts
from pyecharts.charts import Sankey

"""
    绘制： task_graph 对应的关系图
          st_matrix 对应的MapTree图 和 桑基图
"""
...
class STDraw(object):
    ...
    def obtain_graph(graph: TaskGraph, width='1500px', height='800px') -> Graph:
        # STask_block - 绿色字体； CTask_block - 蓝色字体
        # 每个block的输入边中，同一种颜色代表输入同一个输入边簇（在输入边簇数量不超过定义的颜色的种类数的前提下）
    ...
    @staticmethod
    def draw_matrix_map_tree(matrix: STMatrix, leaf_depth=1, width=900, height=500, out_path='./st_matrix_draw_map_tree.html'):
    ...
    @ staticmethod
    def draw_matrix_sankey(matrix: STMatrix, width=2000, height=500, out_path='./st_matrix_draw_sankey.html'):
        c = STDraw.obtain_matrix_sankey(matrix=matrix, width=width, height=height)
        c. render(out_path)
        ...
        for i in range(table_row_value):
            if int(i%4) == 0:
                ...
                if int(i//4) == 0:
                    ...
                    rows[i][0] = 'phase' + ' ' + str(int(i//4) - 1) + ' - Soma1'
            elif int(i%4) == 2:
                if int(i//4) == 0:
                    rows[i][0] = 'phase_group'
                else:
                    rows[i][0] = 'phase' + ' ' + str(int(i//4) - 1) + ' - Soma2'
            elif int(i%4) == 3:
                if int(i//4) == 0:
                    rows[i][0] = 'core'
                else:
                    rows[i][0] = 'phase' + ' ' + str(int(i//4) - 1) + ' - memory'    
    ...
    @ staticmethod
    def draw_matrix_table(matrix: STMatrix, out_path='./st_matrix_draw_sankey.html'):
        table = STDraw.obtain_matrix_table(matrix=matrix)
        table. render(out_path)

    @staticmethod    
    def draw_graph_table(matrix: STMatrix, graph:TaskGraph, width='1500px', height='800px', out_path='./st_matrix_draw_merge.html'):
