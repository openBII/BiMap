...
import src.simulator.resource_simulator.evaluation_model.area.supply_chain_model as scm
...
def calc_cache_sram_area_mm2(capacity_bytes, sram_bitcell_area_mm2, max_cache_unit_bytes=(2**19)):
    if capacity_bytes > max_cache_unit_bytes:  # 512K
...
def calc_compute_core_area_mm2(sa_dim_x: int,
                               sa_dim_y: int,
                               cache_size_bytes: int,  # B
                               num_registers: int,
                               register_bitwidth: int,
                               num_rdwr_ports: int,
                               sa_bitwidth: 'str' = 'fp16',
                               process_node: str = '7nm',
                               vector_int32_count: int = 0,
                               vector_fp16_count: int = 0,
                               vector_fp32_count: int = 0,
                               vector_fp64_count: int = 0,
                               num_reg_files: int = 1,
                               sublane_count: int = 1
                               ):
    # each sublane has a SA and vector unit. a core is made up of sublanes.
    transistor_density_mil_mm2, sram_density_bitcell_mm2 = find_logic_sram_transistor_density(process_node)
    per_sublane_area_mm2 = 0
    # per_sublane_control_area_mm2 = per_sublane_control_dict[device_brand] / 1e6 / transistor_density_mil_mm2
    # per_sublane_area_mm2 += (vector_width * per_sublane_control_area_mm2)
...
if __name__ == "__main__":
    ...
    with open("src/simulator/resource_simulator/evaluation_model/area/GA100.json", "r") as f:
    ...
    compute_area = calc_compute_chiplet_area_mm2(configs_dict)
    io_area = calc_io_die_area_mm2(configs_dict)
    print(
